...
[Transformers e Attention	4](#transformers-e-attention)

[Core	5](#core)

[Tokenizer (tokenizer.py)	5](#tokenizer-(tokenizer.py))

[1. Mapeamento (stoi e itos)	6](#1.-mapeamento-(stoi-e-itos))

[2. Tokens Especiais	6](#2.-tokens-especiais)

[3. O método _tokenize (quebra de texto)	6](#3.-o-método-_tokenize-(quebra-de-texto))

[4. encode e decode	6](#4.-encode-e-decode)

[Exemplo de encode e decode	7](#exemplo-de-encode-e-decode)

[5. O método _detokenize (reconstrução textual)	7](#5.-o-método-_detokenize-(reconstrução-textual))

[**Model (model.py)	8**](#model-(model.py))

[1. GPTConfig	8](#1.-gptconfig)

[2. GPTModel (Arquitetura Principal)	8](#2.-gptmodel-(arquitetura-principal))

[token_emb	8](#token_emb)
...
[3. O método forward	9](#3.-o-método-forward)

[Soma dos embeddings	9](#soma-dos-embeddings)

[Criação da máscara causal (Causal Mask)	9](#criação-da-máscara-causal-(causal-mask))

[Processamento pelo Transformer	9](#processamento-pelo-transformer)

[Saída do modelo	9](#saída-do-modelo)

[**Config (config.py)	10**](#config-(config.py))

[O config.py funciona como o centro de controle técnico do projeto. É nele que ficam definidas tanto as configurações de execução quanto os parâmetros responsáveis pelo comportamento do modelo durante a geração de texto.	10](#o-config.py-funciona-como-o-centro-de-controle-técnico-do-projeto.-é-nele-que-ficam-definidas-tanto-as-configurações-de-execução-quanto-os-parâmetros-responsáveis-pelo-comportamento-do-modelo-durante-a-geração-de-texto.)

[1. Localização do Modelo e Hardware	10](#1.-localização-do-modelo-e-hardware)

[MODEL_PATH	10](#model_path)

[DEVICE	10](#device)

[**Parâmetros de Geração (GEN_PARAMS)	10**](#parâmetros-de-geração-(gen_params))
...
[repetition_penalty	11](#repetition_penalty)

[frequency_penalty	11](#frequency_penalty)

[**Treinando o Modelo	12**](#treinando-o-modelo)

[Para realizar o treinamento e o fine tuning do modelo, foram desenvolvidos dois arquivos Jupyter Notebook que podem ser executados tanto no Google Colab quanto no Kaggle.	12](#stop_loss-=-1.0)

[Na pasta train, temos:	12](#stop_loss-=-1.0)

[● train.ipynb	12](#stop_loss-=-1.0)

[● fine_tuning.ipynb	12](#stop_loss-=-1.0)

[O arquivo train.ipynb mantém praticamente a mesma estrutura principal do projeto — utilizando model.py e tokenizer.py — porém, o foco aqui está no loop de treinamento, que é responsável pelo processo de aprendizado da IA.	12](#stop_loss-=-1.0)

[Além do treinamento em si, o código inclui mecanismos de controle para evitar problemas comuns em modelos pequenos, principalmente o overfitting, quando a IA deixa de aprender padrões gerais e passa apenas a memorizar os textos do dataset.	12](#stop_loss-=-1.0)

[**1. Parada de Emergência (STOP_LOSS)	12**](#1.-parada-de-emergência-(stop_loss))

[Durante o treinamento, o código monitora continuamente o valor de erro (loss) do modelo.	12](#stop_loss-=-1.0)

[Caso o loss fique abaixo de 1.0, o treinamento é interrompido automaticamente.	12](#caso-o-loss-fique-abaixo-de-1.0,-o-treinamento-é-interrompido-automaticamente.)

[STOP_LOSS = 1.0	12](#stop_loss-=-1.0)

[**2. Checkpoints Automáticos (SAVE_AT_LOSS)	12**](#2.-checkpoints-automáticos-(save_at_loss))

[**3. O Ciclo de Aprendizado (Forward & Backward)	13**](#3.-o-ciclo-de-aprendizado-(forward-&-backward))

[Forward	13](#forward)

[Loss	13](#loss)

[Backward	13](#backward)

[Gradient Clipping	13](#gradient-clipping)

[**4. Monitoramento Visual do Treinamento	14**](#4.-monitoramento-visual-do-treinamento)

[LOSS > 3.5	14](#loss->-3.5)

[LOSS > 1.5 e 2.5	14](#loss->-1.5-e-2.5)

[LOSS < 1.0	14](#loss-<-1.0)

[**5. Geração de Amostras em Tempo Real	14**](#5.-geração-de-amostras-em-tempo-real)

[**Pré-Treinamento com Dados Brutos	15**](#pré-treinamento-com-dados-brutos)

[**Fine Tuning	16**](#fine-tuning)

[Executando o Fine Tuning	16](#executando-o-fine-tuning)

[MODEL_PATH	16](#model_path-1)

[FINETUNE_TXT	17](#finetune_txt)

[**Inferência	17**](#inferência)

[**Executando a Aplicação	17**](#executando-a-aplicação)

[1. Execução via Terminal (main.py)	17](#1.-execução-via-terminal-(main.py))

[2. Execução via Interface Web (app.py)	18](#2.-execução-via-interface-web-(app.py))

[REFERÊNCIAS:	19](#referências:)
